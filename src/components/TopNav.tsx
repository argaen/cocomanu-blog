...
import TextLogo from '@/components/svg/Logo';
          ...
          <div className="relative flex h-16 items-center justify-between">
            <div className="absolute inset-y-0 left-0 flex items-center pl-2">
              <Link href="/" aria-label="Navigate to home">
                <TextLogo className="size-8 sm:size-36 py-2 fill-primary w-auto" />
              </Link>
            </div>
                  ...
                  {navigation.map((item) => (
                    <a
                      ...
                      className={classNames(
                        path === item.href ? 'bg-black-sand text-white-water' : 'hover:text-background hover:bg-primary',
                        'text-black-sand rounded-md px-3 py-2 font-medium transition-colors ease-in-out duration-500',
                      )}
                    >
                      {item.name}
                    </a>
